...
export const AuthPage: React.FC<AuthPageProps> = ({ authMode, onSuccess, onBack }) => {
    ...
    const handleOAuth = async (provider: 'google' | 'github') => {
        ...
        try {
            ...
            // OAuth redirects to Google/GitHub, so this code runs right before redirect
            // The browser will navigate away, so we don't need to do anything here
        } catch (err: any) {
            ...
            <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex items-center justify-center p-4">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
                    ...
                    <p className="text-slate-600 mb-6">
                        Supabase is not set up. Please configure <code className="bg-slate-100 px-2 py-1 rounded text-sm">VITE_SUPABASE_URL</code> and <code className="bg-slate-100 px-2 py-1 rounded text-sm">VITE_SUPABASE_ANON_KEY</code> in your environment.
                    </p>
                    <button
                        onClick={onBack}
                    ...
                    <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-100">
                        ...
                        <div className="space-y-3 mb-6">
                            <button
                                ...
                                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border-2 border-slate-200 rounded-lg font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                ...
                                {isOAuthLoading === 'google' ? 'Redirecting to Google...' : 'Continue with Google'}
                            </button>

                            <button
                                onClick={() => handleOAuth('github')}
                                disabled={isOAuthLoading !== null}
                                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-slate-900 border-2 border-slate-900 rounded-lg font-medium text-white hover:bg-slate-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                ...
                                {isOAuthLoading === 'github' ? 'Redirecting to GitHub...' : 'Continue with GitHub'}
                            </button>
                        </div>
